...
async def test_handle_commits_on_success():
    ...
    envelope = DomainEventEnvelope(event_type=PAYMENT_REQUESTED, payload={})

    with patch(
        "app.adapters.inbound.events.handlers.ProcessPaymentRequestedUseCase"
    ) as uc_cls:
        uc_instance = AsyncMock()
        uc_cls.return_value = uc_instance
        handler = make_payment_requested_handler(session_factory, events, gateway)
