...
describe('booking API uncovered flows', () => {
  ...
  it('cancelBooking sends POST with encoded id', async () => {
  ...
  it('abandonCart sends POST to /abandon-cart with encoded id', async () => {
    const detail: BookingDetail = {
      id: 'booking-id',
      status: 'EXPIRED',
      checkin: '2026-06-01',
    ...
    const result = await abandonCart('booking/id');
    const [url, options] = vi.mocked(global.fetch).mock.calls[0] as [string, RequestInit];

    expect(result).toEqual(detail);
    expect(url).toContain('/api/v1/booking/bookings/booking%2Fid/abandon-cart');
    expect(options.method).toBe('POST');
    expect(options.credentials).toBe('include');
  });
