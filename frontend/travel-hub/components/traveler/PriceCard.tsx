  ...
  return (
    <Paper variant="outlined" sx={{ borderRadius: 3, p: 3, position: 'sticky', top: 24 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 0.5, mb: 2 }}>
        {minPrice != null ? (
          <>
            <Typography variant="h5" fontWeight={700}>${minPrice.toLocaleString()}</Typography>
            <Typography variant="body2" color="text.secondary">/ night</Typography>
          </>
        ) : (
          <Typography variant="body2" color="text.secondary">Price not available</Typography>
        )}
      </Box>
      ...
      {minPrice != null && nights > 0 && (
        <>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              <Typography variant="body2" color="text.secondary">Cleaning fee</Typography>
      ...
      <Box sx={{ mt: 2, pt: 2, borderTop: '1px solid', borderColor: 'divider', display: 'flex', alignItems: 'center', gap: 1 }}>
        <Box
          sx={{
            width: 36, height: 36, borderRadius: '50%', bgcolor: 'grey.300',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 16, color: 'grey.600',
          }}
        >
          🏨
        </Box>
        <Box>
          <Typography variant="caption" color="text.secondary">Hosted by</Typography>
