    ...
    >
      ...
      <Box sx={{ flex: 1, p: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, color: 'text.secondary' }}>
          ...
          <Typography variant="body2">{room.capacity} {room.capacity === 1 ? 'guest' : 'guests'}</Typography>
        </Box>
        ...
        {room.amenities.length > 0 && (
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
            {room.amenities.slice(0, 4).map((a) => (
              <Chip key={a.code} label={a.name} size="small" variant="outlined" sx={{ fontSize: '0.7rem', height: 22 }} />
            ))}
          </Box>
        )}
        ...
        {activePlans.slice(0, 2).map((plan) => (
          <Box key={plan.id} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
            <Box>
